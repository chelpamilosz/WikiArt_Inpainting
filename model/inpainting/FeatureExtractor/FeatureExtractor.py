...
class FeatureExtractor():
    ...
    def single_features_extract(self, image, n_components_reduced: int = 32):
        ...
        # Wczytanie zapisanego modelu PCA
        pca = load(r'C:\vscodeProjects\WikiArt_Inpainting\model\inpainting\Models\pca_model.joblib')

        # Wczytanie zapisanego modelu scalera
        scaler = load(r'C:\vscodeProjects\WikiArt_Inpainting\model\inpainting\Models\scaler_model.joblib')
    ...
    def multi_features_extract(self, dataloader: DataLoader, n_components_reduced: int = 32, file_name: str = "extracted_features", pca_model_path=None, scaler_model_path=None):
            ...
            for _, batch in enumerate(tqdm(dataloader, desc="Processing batches")):
                ...
                if not is_pca_fitted:
                    ...
                    if pca_model_path is None:  # Jeśli model PCA nie został załadowany, dopasowujemy go na nowych danych
                        ...
                        dump(pca, '/content/drive/MyDrive/Models/pca_model.joblib')  # Zapis lokalny modelu PCA
                        print(f"Model PCA zapisany w: /content/drive/MyDrive/Models/pca_model.joblib")
        ...
        if scaler_model_path is None:
            ...
            dump(scaler, '/content/drive/MyDrive/Models/scaler_model.joblib')  # Zapis lokalny modelu scaler
            print(f"Model scaler zapisany w: /content/drive/MyDrive/Models/scaler_model.joblib")
        ...
        save_file_name = f'/content/drive/MyDrive/Result/{file_name}_features_and_indices.npz'
